...
function _addSymbolToNgModuleMetadata(
  ...
  if (Array.isArray(node)) {
    ...
    const loonaModule = nodeArray.find(
      node =>
        (node.getText().includes('LoonaModule.forRoot') &&
          symbolName.includes('LoonaModule.forRoot')) ||
        (node.getText().includes('LoonaModule.forChild') &&
          symbolName.includes('LoonaModule.forChild')),
    );

    if (loonaModule && symbolName.includes('LoonaModule')) {
      const loonaArgs = (loonaModule as any).arguments.shift();

      if (
        loonaArgs &&
        loonaArgs.kind === ts.SyntaxKind.ArrayLiteralExpression
      ) {
        const loonaElements = (loonaArgs as ts.ArrayLiteralExpression).elements;
        const [, stateSymbol] = (<any>symbolName).match(/\[(.*)\]/);

        let epos;
        if (loonaElements.length === 0) {
          epos = loonaArgs.getStart() + 1;
          return [new InsertChange(ngModulePath, epos, stateSymbol)];
        } else {
          const lastState = loonaElements[
            loonaElements.length - 1
          ] as ts.Expression;
          epos = lastState.getEnd();
          // Get the indentation of the last element, if any.
          const text: any = lastState.getFullText(source);

          let stateInsert: string;
          if (text.match('^\r?\r?\n')) {
            stateInsert = `,${text.match(/^\r?\n\s+/)[0]}${stateSymbol}`;
          } else {
            stateInsert = `, ${stateSymbol}`;
          }

          return [new InsertChange(ngModulePath, epos, stateInsert)];
        }
      } else {
        return [];
      }
